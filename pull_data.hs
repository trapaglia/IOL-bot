...
loadCreds :: IO (Maybe String)
loadCreds= do
    exists <- doesFileExist ".env"
    if exists then Just <$> readFile "token.txt" else return Nothing
...
main :: IO ()
main = do
    token <- authenticate "MIUSUARIO" "MICONTRASEÑA"
    case token of
...
getCredentials = do
    ...
    user <- lookupEnv "API_USERNAME"
    pass <- lookupEnv "API_PASSWORD"
    return (user, pass)
